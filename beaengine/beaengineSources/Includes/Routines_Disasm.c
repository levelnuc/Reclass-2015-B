...
{
    size_t i = 0;
    if (!strcmp_r(pFormat,"%.2X")) MyNumber = MyNumber & 0xFF;
    if (!strcmp_r(pFormat,"%.4X")) MyNumber = MyNumber & 0xFFFF;
    if (!strcmp_r(pFormat,"%.8X")) MyNumber = MyNumber & 0xFFFFFFFF;
...
void __bea_callspec__ BuildCompleteInstruction(PDISASM pMyDisasm)
{
    ...
    if ((*pMyDisasm).Prefix.RepnePrefix == InUsePrefix) {
            ...
            i = strlen_r((char*) &(*pMyDisasm).CompleteInstr);
    }
